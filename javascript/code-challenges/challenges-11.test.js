...
function transformToLis(obj){
  ...
  result.push(`<li>name: ${obj.name}</li>`) 
  result.push(`<li>age: ${obj.age}</li>`) }
  else result = [];
  return result;
};
...
const count = (target, input) => {
  let result = 0
  const newarr = input.map(item => item.filter(item => item == target));
...
const totalSum = (input) => {
  ...
  const newarr = input.map(item => item.filter(item => item == 6));
  let result = `${newarr}`.replace(/,/ig,'')
...
let findMaleAndFemale = (data) => {
  ...
  return result.toString().replace(/,/g,' and ')
...
combination of filter, map and reduce to return the name of the character who is the shortest in height.
------------------------------------------------------------------------------------------------ */
